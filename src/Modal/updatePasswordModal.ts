...
export const updatePasswordService = {
  async sendOtp(email: string, session: any) {
    ...
    await prisma.user.update({
      where: { email },
      data: { updateOtp: otp, updateOtpExpire: expiry },
    });
...
<body>
  ...
  async verifyOtp(otp: string, session: any) {
    ...
    if (!user || !user.updateOtp || !user.updateOtpExpire)
      throw new Error("Invalid request");

    if (user.updateOtp !== otp) throw new Error("Invalid OTP");
    if (Date.now() > Number(user.updateOtpExpire))
